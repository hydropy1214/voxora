--[[
  CallsPsy AMD (Answering Machine Detection) Script
  ...
  Reads channel variables:
    ...
    callspsy_call_log_id      — CallLog UUID
    callspsy_audio_file       — Absolute path to main audio (shared volume /app/uploads/...)
...
local call_log_id = session:getVariable("callspsy_call_log_id")  or ""
local call_uuid   = session:getVariable("uuid")                   or ""
local audio_file  = session:getVariable("callspsy_audio_file")   or ""
local vm_audio    = session:getVariable("callspsy_voicemail_audio") or ""
local amd_action  = session:getVariable("callspsy_amd_action")   or "PLAY_ON_HUMAN"

freeswitch.consoleLog("INFO",
  string.format("[CallsPsy AMD] uuid=%s campaign=%s log=%s action=%s\n",
...
local function fire(event_type, data)
  local e = freeswitch.Event("CUSTOM", "callspsy::" .. event_type)
...
local function play(file_path)
  ...
  if not f then
    freeswitch.consoleLog("WARNING",
      string.format("[CallsPsy AMD] Audio file not found: %s\n", file_path))
    return false
  end
  f:close()

  freeswitch.consoleLog("INFO",
    string.format("[CallsPsy AMD] Playing: %s\n", file_path))
...
if not session:ready() then
  freeswitch.consoleLog("WARNING", "[CallsPsy AMD] Session not ready, exiting\n")
  return
end
...
if spandsp_result then
  ...
  freeswitch.consoleLog("INFO",
    string.format("[CallsPsy AMD] SpanDSP result: %s (tone=%dms)\n", amd_result, amd_tone_ms))
...
elseif tonumber(session:getVariable("amd_tone_length") or "0") > 0 then
  ...
  amd_result  = "MACHINE"
  freeswitch.consoleLog("INFO",
    string.format("[CallsPsy AMD] Tone detected: %dms → MACHINE\n", amd_tone_ms))

else
  -- Method 3: Default to HUMAN
  freeswitch.consoleLog("INFO", "[CallsPsy AMD] No detection signal → defaulting to HUMAN\n")
end
...
elseif amd_result == "MACHINE" then
  ...
  if amd_action == "VOICEMAIL_DROP" then
    ...
    if amd_tone_ms == 0 then
      freeswitch.consoleLog("INFO", "[CallsPsy AMD] Waiting for VM beep...\n")
      wait_for_beep(5000)
    end
  ...
  elseif amd_action == "HANGUP_ON_MACHINE" then
    freeswitch.consoleLog("INFO", "[CallsPsy AMD] Machine detected → hanging up\n")
...
elseif amd_result == "FAX" then
  ...
  freeswitch.consoleLog("INFO", "[CallsPsy AMD] FAX detected → hanging up\n")
  session:execute("hangup", "NORMAL_CLEARING")
  return
